#!/usr/bin/env bash
# init-spec.sh — Scaffold a review spec for the codex-review skill.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SKILL_DIR="$(dirname "$SCRIPT_DIR")"
TEMPLATE="$SKILL_DIR/templates/review-spec.template.md"
...
if [ "$MODE" = "project" ]; then
  TARGET_DIR="$REPO_ROOT/.codex-review"; TARGET="$TARGET_DIR/spec.md"
else
  TARGET_DIR="$REPO_ROOT/.claude"; TARGET="$TARGET_DIR/review-spec.md"
fi
...
cp "$TEMPLATE" "$TARGET"
echo "✓ Created codex-review spec at: $TARGET"
