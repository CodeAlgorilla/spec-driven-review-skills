...
# bundle when needed.
#
# Spec inline budget: 60000 chars by default (CODEX_REVIEW_SPEC_BUDGET
...
# Spec. Inline budget is in chars; override with CODEX_REVIEW_SPEC_BUDGET.
SPEC_BUDGET="${CODEX_REVIEW_SPEC_BUDGET:-60000}"
case "$SPEC_BUDGET" in
...
if [ -n "$SPEC_TEXT" ]; then
  ...
  RELATED_SECTION=$(echo "$SPEC_FULL" | awk '
...
render_context() {
  ...
  if [ -n "$SPEC_TEXT" ]; then
    ...
    if [ "$SPEC_TRUNCATED" -eq 1 ]; then
      echo "**SPEC TRUNCATED:** only the first $SPEC_BUDGET of ${#SPEC_FULL} chars are shown above. The FULL spec is at \`$SPEC_ABS\` — read it in full (read-only shell, e.g. \`cat\`) before reviewing."
      echo ""
    fi
  fi
